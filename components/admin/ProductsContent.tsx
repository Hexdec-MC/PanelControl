...
export default function ProductsContent({ isAddProductOpen, setIsAddProductOpen }: ProductsContentProps) {
  ...
  const handleSaveProduct = async () => {
    const { name, price, price_mayor, in_stock, unit, description, category, image_file } = form

    if (!name || !price || !price_mayor || !in_stock || !unit) {
      setErrorMessage('Por favor, completa los campos obligatorios: nombre, precio minorista, precio mayorista, cantidad y unidad.')
      return
    }

    const parsedPrice = parseFloat(price)
    const parsedPriceMayor = parseFloat(price_mayor)
    const parsedInStock = parseInt(in_stock, 10)
    if (isNaN(parsedPrice) || isNaN(parsedPriceMayor) || isNaN(parsedInStock)) {
      setErrorMessage('Los precios y la cantidad deben ser valores numéricos válidos.')
      return
    }
    ...
    try {
      if (selectedProduct) {
        ...
        setProducts(products.map((p) =>
          p.id === selectedProduct.id
            ? { ...p, name, price: parsedPrice, 'price-mayor': parsedPriceMayor, in_stock: parsedInStock, unit, description, category, image_url }
            : p
        ))
      } else {
        const { error } = await supabase
  ...
  const handleEdit = (product: any) => {
    setForm({
      name: product.name || '',
      price: product.price ? product.price.toString() : '',
      price_mayor: product['price-mayor'] ? product['price-mayor'].toString() : '0', // Fallback to '0'
      in_stock: product.in_stock ? product.in_stock.toString() : '',
      unit: product.unit || '',
      description: product.description || '',
      category: product.category || '',
  ...
  const handlePrice = async (product: any) => {
    const newRetailPrice = prompt(`Actualizar precio minorista de: ${product.name}`, product.price || '0')
    const newWholesalePrice = prompt(`Actualizar precio mayorista de: ${product.name}`, product['price-mayor'] || '0')
    if (newRetailPrice !== null && newWholesalePrice !== null) {
      const parsedRetailPrice = parseFloat(newRetailPrice)
      const parsedWholesalePrice = parseFloat(newWholesalePrice)
      if (isNaN(parsedRetailPrice) || isNaN(parsedWholesalePrice)) {
      ...
      try {
        const { error } = await supabase
          .from('producto')
          .update({ price: parsedRetailPrice, 'price-mayor': parsedWholesalePrice })
          .eq('id', product.id)

        if (error) throw error

        setProducts(products.map((p) =>
          p.id === product.id ? { ...p, price: parsedRetailPrice, 'price-mayor': parsedWholesalePrice } : p
        ))
      } catch (error: any) {
        alert(`Error al actualizar los precios: ${error.message}`)
      }
    }
  }
        ...
        <Dialog open={isAddProductOpen} onOpenChange={(open) => {
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="grid grid-cols-2 gap-2 max-h-[60vh] overflow-y-auto">
              ...
              <div>
                <Label htmlFor="price_mayor">Precio Mayorista</Label>
                <Input id="price_mayor" value={form.price_mayor} onChange={handleFormChange} placeholder="$0.00" />
              </div>
              <div>
                <Label htmlFor="in_stock">Cantidad</Label>
                <Input id="in_stock" type="number" value={form.in_stock} onChange={handleFormChange} placeholder="0" />
          ...
          <TableBody>
            {filteredProducts.map((product) => (
              <TableRow key={product.id}>
                ...
                <TableCell>${product['price-mayor'] ? product['price-mayor'].toFixed(2) : '0.00'}</TableCell>
                <TableCell>{product.in_stock || '0'}</TableCell>
                <TableCell>{product.unit || 'N/A'}</TableCell>
                <TableCell>
                  <DropdownMenu>
