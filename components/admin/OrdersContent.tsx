...
import { Button } from '../ui/button'
import { createClient } from '@/utils/supabase/client'
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '../ui/tooltip' // Hypothetical tooltip component
...
// Define el texto para el botón y el tooltip
const getActionText = (currentStatus: string) => {
  const nextStatus = getNextStatus(currentStatus)
  return currentStatus === 'Entregado' ? 'Estado final alcanzado' : `Marcar como ${nextStatus}`
}
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Estado</TableHead>
              <TableHead>Fecha</TableHead>
              <TableHead>Cambiar Estado de Pedido</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {filteredOrders.map((order: any) => (
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          ...
                          aria-label={getActionText(order.status)}
                          title={getActionText(order.status)}
                        >
                          {getActionText(order.status)}
                        </Button>
                      </TooltipTrigger>
                      <TooltipContent>
                        <p>{getActionText(order.status)}</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </TableCell>
